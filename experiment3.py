...
def compute_mutual_information(model, dataset, device, samples_per_value=20):
    model.eval()
    factor_sizes = dataset.factor_sizes
    K = len(factor_sizes)
    latent_dim = model.latent_dim
    mi_matrix = np.zeros((latent_dim, K))

    n_bins = 20
    bin_edges = np.linspace(-3, 3, n_bins + 1)

    # Helper to convert factor indices to values
    def indices_to_values(factor_indices):
        shape_idx = factor_indices[0]
        posX = dataset.x_vals[factor_indices[1]]
        posY = dataset.y_vals[factor_indices[2]]
    ...
    for k in range(K):
        num_values = factor_sizes[k]
        samples_by_value = []
        for v in range(num_values):
            latents = []
            for _ in range(samples_per_value):
                f = [np.random.randint(0, s) for s in factor_sizes]
                f[k] = v
                # Convert indices to actual values and draw shape
                shape_idx, posX, posY, scale, rot = indices_to_values(f)
                img = dataset._draw_shape(shape_idx, posX, posY, scale, rot).unsqueeze(0).unsqueeze(0).to(device)
                with torch.no_grad():
                    mu, _ = model.encode(img)
                latents.append(mu.cpu().numpy().flatten())
            latents = np.array(latents)
            samples_by_value.append(latents)

        p_v = 1.0 / num_values
        for j in range(latent_dim):
            all_z = np.concatenate([samples_by_value[v][:, j] for v in range(num_values)])
            hist_all, _ = np.histogram(all_z, bins=bin_edges, density=True)
            hist_all += 1e-10
            mi = 0.0
            for v in range(num_values):
                z_v = samples_by_value[v][:, j]
                hist_v, _ = np.histogram(z_v, bins=bin_edges, density=True)
                hist_v += 1e-10
                kl = np.sum(hist_v * np.log(hist_v / hist_all)) * (bin_edges[1] - bin_edges[0])
                mi += p_v * kl
            mi_matrix[j, k] = mi
    return mi_matrix
...
# -------- training function --------
def train_beta_vae(model, train_loader, optimizer, beta, device):
    model.train()
    ...
    for data, _ in train_loader:
        ...
        loss, bce, kld = beta_vae_loss(recon, data, mu, logvar, beta)
        loss.backward()
        optimizer.step()
...
# -------- experiment runner --------
def run_representation_experiment(beta, device, train_loader, val_loader, dataset_for_metric, epochs=200):
    model = BetaVAE(latent_dim=10).to(device)
    optimizer = optim.Adam(model.parameters(), lr=1e-3)
    for epoch in range(1, epochs + 1):
        train_loss, _, _ = train_beta_vae(model, train_loader, optimizer, beta, device)
        if epoch % 10 == 0:
            print(f"β={beta} Epoch {epoch:3d}: Loss={train_loss:.4f}")

    disent_score = compute_disentanglement_metric(model, dataset_for_metric, device)
    mi_matrix = compute_mutual_information(model, dataset_for_metric, device, samples_per_value=20)
    return model, {"beta": beta, "disentanglement_accuracy": round(disent_score, 4),
...
if __name__ == '__main__':
    ...
    full_dataset = Shapes2DDataset(samples_per_shape=3000)
    ...
    for beta in betas:
        model, res = run_representation_experiment(beta, device, train_loader, val_loader, full_dataset, epochs=200)
        results.append(res)
    ...
    with open("representation_comparison_results.json", "w") as f:
        json.dump(output, f, indent=2)

    for r in results:
        print(f"β={r['beta']:.1f}  DisentAcc={r['disentanglement_accuracy']:.4f}")
        mi = np.array(r["mutual_information"])
        print("Mutual Information matrix (latent x factor):")
        print(np.round(mi, 3))