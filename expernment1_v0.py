...
def compute_disentanglement_metric(model, dataset, device, num_train=5000, num_eval=1000, batch_size=64):
    ...
    # generate training pairs for classifier
...
# -------- experiment runner --------
def run_beta_experiment(beta, device, train_loader, val_loader, dataset_for_metric, epochs=30):
    ...
    return {"beta": beta, "mse": round(mse, 6), "disentanglement_accuracy": round(disent_score, 4)}
...
if __name__ == '__main__':
    ...
    for beta in betas:
        res = run_beta_experiment(beta, device, train_loader, val_loader, full_dataset, epochs=200)
        results.append(res)
    ...
    with open("beta_vae_experiment_results.json", "w") as f:
        json.dump(output, f, indent=2)

    for r in results:
        print(f"β={r['beta']:5.1f}  MSE={r['mse']:.6f}  DisentAcc={r['disentanglement_accuracy']:.4f}")