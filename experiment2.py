...
# -------- Synthetic 2D Shapes Dataset (with variable rotation density) --------
class Shapes2DDataset(Dataset):
    def __init__(self, samples_per_shape=1000, img_size=64, rot_step=1):
        ...
        full_rot_vals = np.linspace(0, 2*np.pi, 40, endpoint=False)
        self.rot_vals = full_rot_vals[::rot_step]  # subsample rotation values
        self.factor_names = ['shape', 'posX', 'posY', 'scale', 'rotation']
        self.factor_sizes = [3, 32, 32, 6, len(self.rot_vals)]
...
# -------- experiment runner --------
def run_continuity_experiment(rot_step, device, epochs=200):
    dataset = Shapes2DDataset(samples_per_shape=3000, rot_step=rot_step)
    ...
    for epoch in range(1, epochs + 1):
        train_loss, _, _ = train_beta_vae(model, train_loader, optimizer, beta=4.0, device=device)
        if epoch % 10 == 0:
            print(f"rot_step={rot_step} Epoch {epoch:3d}: Loss={train_loss:.4f}")

    mse = compute_mse(model, val_loader, device)
    disent_score = compute_disentanglement_metric(model, dataset, device)
    return {"rot_step": rot_step, "rot_values": len(dataset.rot_vals), "mse": round(mse, 6),
            "disentanglement_accuracy": round(disent_score, 4)}
...
if __name__ == '__main__':
    ...
    rot_steps = [1, 2, 4, 8]  # 40, 20, 10, 5 rotation values
    results = []
    start_time = time.time()

    for step in rot_steps:
        res = run_continuity_experiment(step, device, epochs=200)
        results.append(res)
    ...
    with open("continuity_experiment_results.json", "w") as f:
        json.dump(output, f, indent=2)

    for r in results:
        print(f"rot_step={r['rot_step']} (values={r['rot_values']})  MSE={r['mse']:.6f}  DisentAcc={r['disentanglement_accuracy']:.4f}")